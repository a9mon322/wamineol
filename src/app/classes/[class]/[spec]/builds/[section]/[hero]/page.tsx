...
export async function generateMetadata({
  ...
  const build = TALENT_BUILDS.find(
    (b) =>
      ...
      b.heroTalentSlug === p.hero
  );
  const section = SECTIONS.find((s) => s.slug === p.section);
  if (!cls || !spec || !build || !section) return {};
  return {
    title: `${cls.name} ${spec.name} ${section.name} ${build.heroTalentName} 빌드 | 와미널`,
    description: `${cls.name} ${spec.name} ${build.heroTalentName} 영웅특성 ${section.name} 특성 빌드`,
  };
}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 md:px-6">
      <nav className="mb-6 text-sm text-muted">
        ...
        {" / "}
        <span className="text-foreground">{build.heroTalentName} 빌드</span>
      </nav>
      ...
      {build.imageUrl && (
        <section className="mb-8">
          ...
          <img
            src={build.imageUrl}
            alt={`${cls.name} ${spec.name} ${build.heroTalentName} 특성 트리`}
